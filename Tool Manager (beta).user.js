...
(function() {
    ...
    if (IS_DETAIL_PAGE) {
        ...
        function showChapterCountPanel(count, isExact) {
            ...
            if (content) {
                content.onclick = () => {
                   if (!isExact) document.querySelector('.volume-list .pagination')?.scrollIntoView({behavior: "smooth"});
                };
                content.style.cursor = isExact ? 'default' : 'pointer';
            }
            if (closeBtn) {
                closeBtn.onclick = (e) => {
                    e.stopPropagation();
    ...
    if (IS_LIST_PAGE) {
        ...
        panel.querySelectorAll('.wd-input, .wd-select, .wd-btn').forEach(el => {
        ...
        async function syncData() {
            ...
            if(!audioOK) showToast("⚠️ Không bật được Audio để tránh tab đóng băng");

            btn.disabled = true;
            let all = [], page = 1, start = 0, hasNext = true, total = 0, old = {}, retries = 0;
            ...
            while (hasNext) {
                msg.innerHTML = `${audioOK ? '🔊 ' : ''}Trang ${page} - ${all.length}/${total||'?'}`;
                btn.innerText = `⏳ ${total ? ((all.length/total)*100).toFixed(0)+'%' : all.length}`;
                ...
                try {
                    ...
                    if (els.length === 0) {
                        if(total > 0 && all.length >= total) {
                            hasNext = false;
                             ...
                             console.warn(`Trang trống tại start=${start}, bỏ qua...`);
                             retries++;
                    ...
                    console.error(e);
                    msg.innerText = `Lỗi mạng (Retry ${retries}/3)...`;
                    retries++;
            ...
            if (total > 0 && all.length < total) { msg.innerHTML = `<span style="color:var(--wd-danger)">⚠️ Thiếu: ${all.length}/${total}</span>`; }
            else msg.innerText = `✅ Xong: ${all.length} truyện.`;
        ...
        function renderList() {
            ...
            bks.forEach(b => {
                ...
                const linkEl = d.querySelector('.wd-item-title');
